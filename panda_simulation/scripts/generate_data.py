...
def create_images():
	...
	img_name = "test"+str(x) #rename img
        renameCommand = "mv ~/arm_testing_ws/src/panda_simulation/data_for_nn/camera_image.jpeg ~/arm_testing_ws/src/panda_simulation/data_for_nn/{}.jpeg".format(img_name) 
	os.system(renameCommand)
	
	#make a file with the title of the z-coord
	makeCommand = "mkdir {}".format(new_Z-0.1224)
	os.system(makeCommand)
	#move the img to the file
	moveCommand = "mv {}.jpeg ~/arm_testing_ws/src/panda_simulation/data_for_nn/{}".format(img_name,new_Z-0.1224)
	os.system(moveCommand)
	
	#add the corresponding z-coord to the label file
	add_label((new_Z - 0.1224))
